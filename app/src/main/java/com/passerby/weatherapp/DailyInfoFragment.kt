...
class DailyInfoFragment : DailyBaseFragment<DailyWeatherModel>() {
    ...
    override fun updateView() {
        mData?.apply {
            ...
            day_temp.text =
                viewContext.getString(R.string.degree_symbol, temp.getAverage().toDegree())
            ...
            day_morn_fl.text =
                viewContext.getString(R.string.degree_symbol, feels_like.morn.toDegree())
            day_daily_temp.text = viewContext.getString(R.string.degree_symbol, temp.day.toDegree())
            day_daily_fl.text =
                viewContext.getString(R.string.degree_symbol, feels_like.day.toDegree())
            day_eve_temp.text = viewContext.getString(R.string.degree_symbol, temp.eve.toDegree())
            day_eve_fl.text =
                viewContext.getString(R.string.degree_symbol, feels_like.eve.toDegree())
            day_night_temp.text =
                viewContext.getString(R.string.degree_symbol, temp.night.toDegree())
            day_night_fl.text =
                viewContext.getString(R.string.degree_symbol, feels_like.night.toDegree())
            day_humidity.text = ("$humidity %")
            val settingPressure = SettingsHolder.pressure
            day_pressure.text = viewContext.getString(
                settingPressure.measureUnitStringRes,
                settingPressure.getValue(pressure.toDouble())
            )
            val settingWindSpeed = SettingsHolder.windSpeed
            day_wind_speed.text = viewContext.getString(
                settingWindSpeed.measureUnitStringRes,
                settingWindSpeed.getValue(wind_speed)
            )
